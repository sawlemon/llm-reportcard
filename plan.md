...
A single root-level app; this repository holds nothing else:

```
LLM_REPORT_CARD.md            data source — at the repo root, alongside the app it feeds
...
more often than the app is developed. The app reaches it via an explicit path passed to the plugin from
`vite.config.ts`, rather than resolving against the Vite root, so the location is stated in exactly one
place.
