...
| Coding | | |
| Instruction-following | | |
| Tool use / agentic | at medium reasoning effort, eventually gets practical research tasks done, including finding stores in a location, researching OLX listings, and finding an iOS video player; gave a solid recommendation when using Exa for research, which Claude Opus 4.8 also agreed with | rarely gets the right answer on the first attempt; typically needs multiple follow-up turns before reaching the correct result; on a PDF task in the ChatGPT app, using the exact same prompt and harness, results were wildly worse than Claude Opus 4.8 — took many tries and still kept making mistakes, overall unsatisfying |
| Context handling | | |
...
| Refusals / safety behavior | | |
| Formatting / output quality | | |
| Other | may be worth trying again for background automation tasks, where the slow response is less disruptive; the more it's used, the more the user likes it — maybe just Stockholm syndrome, but the verdict is the model is decent enough | the suggestion that it runs on Chinese hardware is unverified; for normal task execution, prefers GPT 5.6 Luna, which is fast enough and follows instructions well enough; frequently makes lots of small mistakes and corrects itself back on track — behavior similar to Opus 5, which the user disliked; low cost is the only justification; now restricted to implementation tasks only, not research, because it makes mistakes multiple times — would rather use Luna max for implementations, especially with Luna maxing making more sense since the Codex subscription nerfing |
