...
| Refusals / safety behavior | | |
| Formatting / output quality | | |
| Other | initial Ox Alpha-era skepticism (“why is everyone overhyping this?”) was overturned after heavy use; very capable despite occasional mistakes; low hallucination rate makes it the user's new favorite model and favorite flash model | implementation only, not for research: made research mistakes and nearly deleted the Hindsight memory database; use frontier models for planning and research, and be careful with destructive operations |
