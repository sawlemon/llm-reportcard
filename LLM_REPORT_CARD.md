...
The two reserved sections at the end of the file are also data. `## Recommendations` records the single
recommended setup per task, and `## Task Verdicts` records one task-specific verdict row per task/model
pair (`| Task | Model | Status | Date | Summary |`, status `preferred` or `care`). A model absent from
`## Task Verdicts` for a task is omitted from that task's list — record only what was actually observed
for that task, and never list a model whose task evidence is negative-only.
...
|---|---|---|---|---|
| Debugging | Claude Sonnet 5 | preferred | 2026-09-03 | Solved the local toolset troubleshooting problem immediately on medium, after GPT 5.6 Sol and GPT-6 Astra failed to diagnose it. |
| Debugging | Claude Opus 5 | care | 2026-09-03 | Strong at root-cause investigation — replicated a cliproxyapi bug from scratch to confirm the fix — but makes more mistakes than Opus 4.8 and is token-hungry. |
| Implement from plan | Claude Sonnet 5 | preferred | 2026-09-03 | The preferred implementer on medium for plans designed by Opus 4.8; that plan-and-implement workflow keeps working out great. |
| Implement from plan | DeepSeek V4 Flash | preferred | 2026-08-21 | Very thorough even when implementing a plan generated by a frontier model, following it through end-to-end. |
| Implement from plan | GPT 5.6 Luna | care | 2026-09-11 | Completed the first-hand implementer test at max effort and got to the point, but the long thinking phase makes output arrive late. |
| Implement from plan | GLM 5.3 Flash | care | 2026-08-30 | Great for implementing things, but frequently makes mistakes when editing files. |
| Planning / architecture | Claude Opus 4.6 | preferred | 2026-08-10 | Plans are very efficient and understandable — straight to the point where Opus 4.8 and Opus 5 overengineer. |
| Planning / architecture | Claude Opus 4.8 | care | 2026-09-11 | Was the planner half of the plan-with-Opus-4.8 / implement-with-Sonnet-5 workflow, but no longer trusted for thinking tasks after repeated refactor mistakes. |
| Planning / architecture | Claude Opus 5 | care | 2026-09-11 | Plans well enough to anchor the plan-with-Opus-5 / implement-with-Luna cheap workflow, but overengineers and is token-hungry. |
| Planning / architecture | GPT 5.6 Tera | care | 2026-09-11 | Planned the LLM skills site build solidly at high reasoning effort, but makes unrequested scope changes without confirmation. |
| Exploratory research | GPT 5.6 Luna | preferred | 2026-09-04 | Preferred for exploratory research when paired with Exa at max effort; genuinely impressive in first-hand use. |
| Exploratory research | Claude Opus 4.6 | care | 2026-08-10 | Can invoke web search through Antigravity, giving it a path to web research; best known for efficient, focused planning. |
| Exploratory research | Claude Opus 4.8 | care | 2026-09-11 | Previously preferred over GPT 5.6 Sol for research, but no longer trusted for thinking tasks. |
| Exploratory research | Claude Opus 5 | care | 2026-09-11 | Consistently does a great job hunting down information and root causes, though reasoning sometimes misses facts and relies on wrong assumptions. |
...
| Speech to text | Parakeet Unified ENG 0.6B | preferred | 2026-08-10 | Current voice-to-text model of choice with very accurate live streaming transcription, though recognition accuracy has been degrading and needs a custom-vocabulary workaround. |
