...
describe('useHashModel', () => {
  ...
  it('preserves the path and query when writing the fragment', () => {
    window.history.replaceState(null, '', '/llm-reportcard/?q=grok');
    const { result } = renderHook(() => useHashModel());

    act(() => result.current[1]('acme-labs--x'));
    expect(window.location.href).toContain('/llm-reportcard/?q=grok#acme-labs--x');

    act(() => result.current[1](null));
    expect(window.location.href).toContain('/llm-reportcard/?q=grok');
    expect(window.location.hash).toBe('');
  });
